...
class ResultHandler:
    ...
    def save(self):
        ...
        # ✅ date string (reuse later)
        date_str = datetime.datetime.now().strftime("%Y-%m-%d")

        # ✅ rename first column header
        final_df.rename(columns={"model": "Models"}, inplace=True)

        # ✅ base path (works for EXE + .py)
        ...
        while True:
            filename = os.path.join(
                RESULT_PATH,
                f"{base_filename}_{date_str}_v{version}.xlsx"
            )
            if not os.path.exists(filename):
                break
            version += 1

        final_df.to_excel(filename, index=False, startrow=1)
